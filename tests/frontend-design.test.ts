...
describe("frontend information architecture", () => {
  ...
  it("shows the redesigned community announcement only once", () => {
    ...
    expect(announcement).toContain('data-community-orbit="outer"')
    expect(announcement).toContain('data-community-orbit="inner"')
    expect(announcement).toContain("icon={CalendarDays}")
    expect(announcement).toContain("icon={GraduationCap}")
    expect(announcement).not.toContain("absolute -right-1 -top-1 size-3 rounded-full")
